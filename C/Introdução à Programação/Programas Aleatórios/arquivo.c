...
//Função para salvar os dados cadastrados em um arquivo texto
...
//Função para salvar os dados cadastrados em arquivo binário
void salvar_em_arquivo_binario(pessoa_t * ponteiro_pessoa, int num_pessoas){
    FILE * arquivo;

    arquivo = fopen("arquivo2.txt","wb");
    if (!arquivo){
        printf ("Erro ao abrir o arquivo!!!\n");
        exit(1);
    }else {
        fprintf (arquivo,"%2d\n", num_pessoas);
        fwrite (ponteiro_pessoa, sizeof(pessoa_t), num_pessoas, arquivo);
        fclose (arquivo);
    }
}
void ler_arquivo_em_binario(pessoa_t * ponteiro_pessoa,int num_pessoas){
    FILE * arquivo;
    ...
    if (!arquivo){
        ...
        fread(ponteiro_pessoa, sizeof (pessoa_t), num_pessoas, arquivo);
        fclose (arquivo);
    }
...
void ler_arquivo(pessoa_t * ponteiro_pessoa,int num_pessoas){
    ...
    if (!arquivo){
        ...
        for (i=0;i<num_pessoas;i++){
            ...
            util_removeQuebraLinhaFinal(ponteiro_pessoa[i].nome);
...
int main (int argc,char ** argv){
    ...
    do {
        ...
        printf ("(1) <CADASTRO>\n(2) <LISTAR EM TABELA>\n(3) <SALVAR INFORMACOES>\n(4)<LER INFORMACOES>\n(5) <SALVAR INFORMACOES EM BINARIO>\n(6) <LER DO ARQUIVO BINARIO>\n(0) <PARAR>\n");
        scanf ("%d", &opcao);
        getchar ();
        ...
        switch (opcao){
            ...
            case 5:{
                salvar_em_arquivo_binario(ponteiro_pessoa,num_pessoas);
                break;
            }
            case 6:{
                ler_arquivo_em_binario(ponteiro_pessoa,num_pessoas);
                break;
            }
            case 0:{
